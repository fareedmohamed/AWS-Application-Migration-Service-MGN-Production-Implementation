...
## 9. End-to-End Migration Timeline (Gantt)

```mermaid
gantt
    dateFormat  YYYY-MM-DD
    title AWS MGN Migration — End-to-End Timeline (Example: Single Wave, 5 Servers)
    section Assessment
    Customer Assessment           :done, assess, 2026-07-20, 3d
    section Discovery
    Discovery & Dependency Mapping :done, disc, after assess, 3d
    section Preparation
    IAM / Network / Staging Setup  :done, prep, after disc, 2d
    Install Replication Agent      :done, agent, after prep, 1d
    section Replication
    Initial Sync                   :active, sync, after agent, 2d
    Continuous Replication / Stabilize :contrep, after sync, 4d
    section Configuration
    Configure Launch Settings      :launch, after contrep, 1d
    section Testing
    Test Launch                    :test, after launch, 1d
    Validation                     :valid, after test, 2d
    Rollback / Fix Issues (buffer)  :rollback, after valid, 1d
    section Cutover
    Final Sync & Cutover Window     :crit, cutover, after rollback, 1d
    Post-Cutover Stabilization      :stabilize, after cutover, 2d
    section Closeout
    Disconnect Replication & Cleanup :cleanup, after stabilize, 1d
    Documentation & Handover         :docs, after cleanup, 1d
```

**Notes on timeline scaling:**
- For **Scenario 1 (20 servers)**, multiply Replication + Testing phases across 3-4 sequential waves rather than compressing into the single-wave timeline above — total calendar time extends, but effort per wave stays similar to this template
- **Initial Sync duration** is the most variable line item — always re-forecast it after the first 2-4 hours of real observed throughput rather than trusting the pre-migration estimate alone
- **Cutover window** itself (final sync + launch + DNS switch) is typically the shortest phase (often under 1-2 hours for app/web tiers, potentially longer for large SQL databases needing service restart + `DBCC CHECKDB`)

---

## 10. Quick-Reference CLI Command Sheet

```bash
# List all source servers and their status
aws mgn describe-source-servers --region us-east-1

# Get details on a single server (status, lag, lifecycle state)
aws mgn describe-source-servers \
  --filters sourceServerIDs=s-0123456789abcdef0

# Update replication settings for a specific server
...
# Start a test launch
aws mgn start-test --source-server-id s-0123456789abcdef0

# Mark server as ready for cutover (after successful test)
aws mgn mark-as-archived --source-server-id s-0123456789abcdef0   # (only after full completion)

# Start cutover
aws mgn start-cutover --source-server-id s-0123456789abcdef0

# Finalize cutover (confirms migration complete, stops treating as "in cutover")
aws mgn finalize-cutover --source-server-id s-0123456789abcdef0

# Disconnect source server from service (post-validation, tears down staging resources)
aws mgn disconnect-from-service --source-server-id s-0123456789abcdef0

# List all launch configuration jobs / history
